...
一个基于 Node.js + MongoDB 的轻量级移动端数据采集平台，专为业务人员外出跑市场时记录客户/供应商信息而设计。支持多维度分类、按天分组、Excel 导出、批量删除、二维码分享，并可通过免费定时任务将每日新增数据推送到微信。

---

## 🎯 项目背景

业务人员通过微信群散乱地发送客户信息，容易造成数据丢失、重复、难以汇总。本系统提供 **手机端快速填报 + 云端持久化存储 + 后台统一管理** 的闭环方案：

- **业务员**：通过手机浏览器填写客户/供应商信息，区分企业/个人，数据实时上传。
- **管理者**：登录后台按天查看所有新增数据，支持导出 Excel 和批量删除，轻松录入公司系统。
- **定时推送**：每天定时将新增数据汇总推送到微信，及时掌握需录入的数据量。
...
| 🏢 **多公司分配** | 可将客户/供应商分配给一个或多个内部公司（公司列表可在前端配置）。 |
...
| 📲 **二维码分享** | 填表页面底部自动生成二维码，业务员可长按保存；后台可查看并下载二维码，方便分发。 |
| ⏰ **定时微信推送** | 通过 cron-job.org + 方糖，每天定时将前一日 20:00 至当日 20:00 的新增数据汇总推送到微信。 |
...
```
customer-api/
├── server.js              # 主程序（API + 认证 + 静态托管 + 推送接口）
...
| `SERVERCHAN_SENDKEY` | 你的方糖 SendKey（如 `SCTxxxxxxxxxxxxxxxxxxxxxxxx`） |
| `CRON_SECRET` | 自定义推送密钥（如 `mysecret123`） |
...
| GET | `/cron/push?secret=xxx` | 触发定时推送（供 cron-job.org 调用） | 密钥保护 |
...
```json
{
  ...
  "idNumber": "91440101MA5XXXXXX", // 企业填信用代码，个人填身份证号（15或18位）
  "contactName": "张三",
  "contactPhone": "13800138000",
  "salesman": "李四",
  "assignedCompanies": ["分公司A", "分公司B"]  // 可多选
}
```
...
6. 点击“📲 二维码”按钮可查看并下载填表二维码，方便分发给业务员。
...
### 1. 获取方糖 SendKey
1. 微信搜索并关注 **“方糖”** 公众号。
2. 访问 [https://sct.ftqq.com](https://sct.ftqq.com) 登录，复制你的 SendKey。
3. 将 SendKey 填入 Render 环境变量 `SERVERCHAN_SENDKEY`。

### 2. 注册 cron-job.org 并创建任务
1. 访问 [https://cron-job.org](https://cron-job.org)，注册免费账号。
2. 点击 **Create Cronjob**，填写：
   - **Title**：每日客商信息推送
   - **URL**：`https://你的域名.onrender.com/api/cron/push?secret=你的CRON_SECRET`
   - **Schedule**：每天，时间设为 **北京时间 20:00**
   - **Timezone**：`Asia/Shanghai`
3. 开启 **Retry on failure**（失败重试），防止 Render 休眠导致首次请求失败。
4. 保存并启用。
...
统计时间段：2026-09-09 20:00 ~ 2026-09-10 20:00
...
| `SERVERCHAN_SENDKEY` | ✅ | 方糖 SendKey |
| `CRON_SECRET` | ✅ | 推送接口的访问密钥，自定义字符串 |
...
1. **免费额度限制**：
   ...
   - 方糖免费版每天限 5 条推送，超额后 API 返回成功但微信不推送，额度每天 0 点重置。

2. **安全性建议**：
   - 使用强密码（大小写+数字+特殊字符），并定期更换。
   - 环境变量中的敏感信息不要写入代码或提交到 GitHub。
